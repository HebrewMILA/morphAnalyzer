 ...
 * 
 * To marshal an object, create an instance of SAXMarshaller and call the
 * serializeElements method of the XMLSerializable object that you want to
 * marshal.
 * 
...
public class SAXMarshaller implements XMLSerializer {
	/**
	 * "Attributes" object that is passed to the startElement event. One object
	 * is reused throughout the marshalling.
	 */
	...
	private final PrefixCallback startPrefixCallback = new PrefixCallback() {
		@Override
		public void onPrefixMapping(String prefix, String nsUri)
				throws SAXException {
	...
	/**
	 * Default {@link NamespacePrefixMapper} implementation used when it is not
	...
	/**
	 * Creates a marshalling context by designating the ContentHandler that
	 * receives generated SAX2 events.
	 */
	public SAXMarshaller(ContentHandler _writer,
			NamespacePrefixMapper prefixMapper, MarshallerImpl _owner) {
		this.writer = _writer;
		this.owner = _owner;
		this.nsContext = new NamespaceContextImpl(
				prefixMapper != null ? prefixMapper
						: defaultNamespacePrefixMapper);
	}

	@Override
	public void childAsAttributes(JAXBObject o, String fieldName)
			throws SAXException {
		if (o == null) {
		...
		owner.context.getGrammarInfo().castToXMLSerializable(o)
	...
	public void childAsBody(JAXBObject o, String fieldName) throws SAXException {
		if (o == null) {
			...
			reportMissingObjectError(fieldName);
			// as a marshaller, we should be generous, so we'll continue to
			// marshal
	...
	public void endAttribute() {
		...
		if (attNamespaceUri.length() == 0) {
			...
			qname = nsContext.declareNamespace(attNamespaceUri, null, true)
					+ ':' + attLocalName;
		}

		attributes.addAttribute(attNamespaceUri, attLocalName, qname, "CDATA",
				textBuf.toString());
	}

	/**
	 * Switches to the "marshal child texts/elements" mode. This method has to
	 * be called after the 1st pass is completed.
	 */
	@Override
	public void endAttributes() throws SAXException {
		...
		_assert(prefix != null); // since we've declared it, it should be
									// available
	...
	@Override
	public String onID(IdentifiableObject owner, String value)
			throws SAXException {
	...
	public String onIDREF(IdentifiableObject obj) throws SAXException {
		...
		if (id == null) {
			reportError(new NotIdentifiableEventImpl(ValidationEvent.ERROR,
	...
	void reconcileID() throws AbortSerializationException {
		...
		for (Iterator itr = idReferencedObjects.iterator(); itr.hasNext();) {
			...
			reportError(new NotIdentifiableEventImpl(ValidationEvent.ERROR,
					Messages.format(Messages.ERR_DANGLING_IDREF,
							o.____jaxb____getId()),
	...
	public void reportError(ValidationEvent ve)
		...
		if (!handler.handleEvent(ve)) {
			if (ve.getLinkedException() instanceof Exception)
				throw new AbortSerializationException(
	 ...
	 * </ol>
	 * 
	 * No two attributes can be marshalled at the same time. Note that the whole
	 * attribute marshalling must be happened after the startElement method and
	 * before the endAttributes method.
	 */
	@Override
	...
	/**
	 * Starts marshalling of an element. Calling this method will push the
	 * internal state into the internal stack.
	 */
	@Override
	public void startElement(String uri, String local) throws SAXException {
		...
		if (isRoot) {
			...
			String[] uris = nsContext.getNamespacePrefixMapper()
	 ...
	 * 
	 * <p>
	 * This method can be called (i) after the startAttribute method and (ii)
	 * before the endAttribute method, to marshal attribute values. If the
	 * method is called more than once, those texts are considered as separated
	 * by whitespaces. For example,
	 * 
	 * <pre>
	 * c.startAttribute();
	 * c.text(&quot;abc&quot;);
	 ...
	 * 
	 * <p>
	 * Similarly, this method can be called after the endAttributes method to
	 * marshal texts inside elements. The same rule about multiple invokations
	 * apply to this case, too. For example,
	 * 
	 * <pre>
	 * c.startElement(&quot;&quot;, &quot;foo&quot;);
	 * c.endAttributes();
	...
	/**
	 * Writes pending text (characters inside elements) to the writer. This
	 * method is called from startElement and endElement.
	 */
