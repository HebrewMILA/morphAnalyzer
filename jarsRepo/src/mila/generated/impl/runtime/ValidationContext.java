...
/**
 * Maintains information that needs to be stored across validations of multiple
 * objects.
 * 
 * Specifically, this object is responsible for:
 * 
 * <ol>
 * <li>detecting a cycle in a content tree by keeping track of objects that were
 * validated.
 * 
 * <li>keeping an instance of NamespaceContextImpl, which is shared by all
 * MSVValidators.
 * 
 * <li>keeping a reference to {@link ValidationErrorHandler}. MSVValidators
 * should use this error handler to report any error.
 * </ol>
 */
class ValidationContext {
	...
	ValidationContext(DefaultJAXBContextImpl _context,
			ValidationEventHandler _eventHandler, boolean validateID) {
	...
	public String onID(XMLSerializable owner, String value) throws SAXException {
		...
		if (!IDs.add(value)) {
			...
			reportEvent(
					jaxbContext.getGrammarInfo().castToValidatableObject(owner),
					Messages.format(Messages.DUPLICATE_ID, value));
		}
	...
	public String onIDREF(XMLSerializable referer, String value)
			throws SAXException {
		if (!validateID)
			return value;
	...
	protected void reconcileIDs() throws SAXException {
		...
		for (Iterator itr = IDREFs.entrySet().iterator(); itr.hasNext();) {
			...
			reportEvent(source,
					new NotIdentifiableEventImpl(ValidationEvent.ERROR,
	...
	/**
	 * Reports an error to the client. This version should be used when an
	 * exception is thrown from sub-modules.
	 */
	public void reportEvent(ValidatableObject source, Exception nestedException)
			throws AbortSerializationException {
		reportEvent(source, new ValidationEventImpl(ValidationEvent.ERROR,
				nestedException.toString(), new ValidationEventLocatorImpl(
	...
	public void reportEvent(ValidatableObject source, String formattedMessage)
			throws AbortSerializationException {
		reportEvent(source, new ValidationEventImpl(ValidationEvent.ERROR,
				formattedMessage, new ValidationEventLocatorImpl(source)));
	}
	...
	/**
	 * Validates the sub-tree rooted at <code>vo</code> and reports any
	 * errors/warnings to the error handler.
	 */
