...
public class MSVValidator implements XMLSerializer, IDContextProvider2 {
	...
	public static void validate(DefaultJAXBContextImpl jaxbCtx,
			ValidationContext context, ValidatableObject vo)
			throws SAXException {
		try {
	...
	/**
	 * namespace URI of dummy elements. TODO: allocate one namespace URI for
	 * this.
	 */
	...
	private MSVValidator(DefaultJAXBContextImpl _jaxbCtx,
			ValidationContext _ctxt, ValidatableObject vo) {
	...
	private void childAsAttributeBody(ValidatableObject vo, String fieldName)
			throws SAXException {
		/*
		 * Dirty quick hack. When we split a schema into fragments, basically
		 * every chlid object needs a place holder in the fragment (so that the
		 * parent schema fragment can correctly validate that the child objects
		 * are at their supposed places.)
		 * 
		 * For example, cconsider the following schema:
		 * 
		 * imagine: <class> <attribute> <list> <oneOrMore> <ref name="bar"/>
		 * </oneOrMore> </list> </attribute> </class>
		 * 
		 * In our algorithm, the corresponding schema fragment will be:
		 * 
		 * <class> <attribute> <list> <oneOrMore>
		 * <value>\u0000full.class.name.of.BarImpl</value> </oneOrMore> </list>
		 * </attribute> </class>
		 * 
		 * If we find a child object inside an attribute (that's why we are in
		 * this method BTW), we generate a class name (with a special marker
		 * \u0000).
		 */

		// put a class name with a special marker \u0000. This char is an
		// invalid
	...
	public void childAsBody(JAXBObject o, String fieldName) throws SAXException {
		...
		final ValidatableObject vo = jaxbContext.getGrammarInfo()
	...
	private void childAsElementBody(Object o, ValidatableObject vo)
			throws SAXException {
		...
		if (o instanceof RIElement) {
			RIElement rie = (RIElement) o;
			if (!child.onAttribute2(rie.____jaxb_ri____getNamespaceURI(),
					rie.____jaxb_ri____getLocalName(),
					rie.____jaxb_ri____getLocalName(), "", null, null, null))

				// this object is not a valid member of the wildcard
				context.reportEvent(target, Messages.format(
						Messages.INCORRECT_CHILD_FOR_WILDCARD,
						rie.____jaxb_ri____getNamespaceURI(),
						rie.____jaxb_ri____getLocalName()));
		}
	...
	public void endAttribute() throws SAXException {
		...
		if (!acceptor.onAttribute2(attNamespaceUri, attLocalName,
				attLocalName /*
							 * we don't have QName , so just use the local name
	...
	public void endAttributes() throws SAXException {
		if (!acceptor.onEndAttributes(null, null)) {
			...
			// causes this error.
			StringRef ref = new StringRef();
			StartTagInfo sti = new StartTagInfo(currentElementUri,
	...
	public void endElement() throws SAXException {
		...
		if (!acceptor.stepForward(child, null)) {
			...
			// report an error
			StringRef ref = new StringRef();
			acceptor.stepForward(child, ref); // force recovery and obtain an
												// error message.
	...
	public void startElement(String uri, String local) throws SAXException {
		...
		StartTagInfo sti = new StartTagInfo(uri, local, local, emptyAttributes,
				this);

		// we pass in an empty attributes, as there is just no way for us to
		// properly re-construct attributes. Fortunately, I know MSV is not
		// using
