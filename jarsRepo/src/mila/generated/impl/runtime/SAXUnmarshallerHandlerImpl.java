 ...
 */
public class SAXUnmarshallerHandlerImpl implements SAXUnmarshallerHandler,
		UnmarshallingContext {
	/**
	 * This flag is set to true at the startDocument event and false at the
	 * endDocument event.
	 * 
	 * Until the first document is unmarshalled, we don't want to return an
	 * object. So this variable is initialized to true.
	 */
	...
	/**
	 * Stack of collectText flag. False means text can be ignored for this
	 * element.
	 * 
	...
	//
	/**
	 * Submitted patchers in the order they've submitted. Many XML vocabulary
	 * doesn't use ID/IDREF at all, so we initialize it with null.
	 */
	...
	/**
	 * Computes the names of possible root elements for a better error
	 * diagnosis.
	 */
	...
	public final void endElement(String uri, String local, String qname)
		...
		// work gracefully with misconfigured parsers that don't support
		// namespaces
		if (uri == null)
			uri = "";
	...
	public Iterator getPrefixes(String uri) {
		...
		return Collections.unmodifiableList(getAllPrefixesInList(uri))
				.iterator();
	}
	...
	public void handleEvent(ValidationEvent event, boolean canRecover)
		...
		if (!canRecover || !recover)
			throw new SAXException(new UnmarshalException(event.getMessage(),
	...
	public void pushAttributes(Attributes atts, boolean collectTextFlag) {
		...
		for (int i = 0; i < atts.getLength(); i++) {
			...
			// <foo xsi:nil="false">some value</foo> is a valid fragment,
			// however
			// we need a look ahead to correctly handle this case.
			// (because when we process @xsi:nil, we don't know what the value
			// is,
			// and by the time we read "false", we can't cancel this attribute
			// anymore.)
			//
	...
	/**
	 * Returns true if we should be collecting characters in the current
	 * element.
	 */
	...
	public void startElement(String uri, String local, String qname,
		...
		if (result == null) {
			...
			UnmarshallingEventHandler unmarshaller = grammarInfo
					.createUnmarshaller(uri, local, this);
			if (unmarshaller == null) {
				// the registry doesn't know about this element.
				//
				// the no.1 cause of this problem is that your application is
				// configuring
				// an XML parser by your self and you forgot to call
				// the SAXParserFactory.setNamespaceAware(true). When this
				// happens, you see
				// the namespace URI is reported as empty whereas you expect
				// something else.
				throw new SAXParseException(Messages.format(
						Messages.UNEXPECTED_ROOT_ELEMENT2, uri, local,
						computeExpectedRootElements()), getLocator());
			}
