...
public class GrammarInfoImpl implements GrammarInfo {
	...
	/**
	 * Map from {@link Class}es that represent content interfaces to
	 * {@link String}s that represent names of the corresponding implementation
	 * classes.
	 */
	...
	public final UnmarshallingEventHandler createUnmarshaller(
		...
		try {
			return ((UnmarshallableObject) impl.newInstance())
	...
	public final Class getDefaultImplementation(Class javaContentInterface) {
		try {
			...
			return Class
					.forName((String) defaultImplementationMap
							.get(javaContentInterface), true, classLoader);
	...
	public final com.sun.msv.grammar.Grammar getGrammar() throws JAXBException {
		try {
			...
			if (is == null) {
				...
				name = '/' + name.substring(0, idx + 1).replace('.', '/')
						+ "bgm.ser";
			...
			com.sun.xml.bind.GrammarImpl g = (com.sun.xml.bind.GrammarImpl) ois
					.readObject();
			ois.close();

			g.connect(new com.sun.msv.grammar.Grammar[] { g }); // connect to
																// itself

			return g;
		} catch (Exception e) {
			throw new JAXBException(
					Messages.format(Messages.UNABLE_TO_READ_BGM), e);
		}
	}
	...
	/**
	 * @return the name of the content interface that is registered with the
